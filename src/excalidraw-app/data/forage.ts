...
export const getExamStudentCommandFromLocalForage = async () => {
    try {
        let command = await STORE_EXAM_STUDENT.getItem("examStudentCommand")
        return command;
    } catch (error: any) {
...
export const getExamStudentFileNameFromLocalForage = async () => {
    try {
        let createFileName = await STORE_EXAM_STUDENT.getItem("createFileName");
...
export const executeExamStudentCommand = async () => {
    try {
        let command = await getExamStudentCommandFromLocalForage() as string;
        let createFileName = await getExamStudentFileNameFromLocalForage() as string;
